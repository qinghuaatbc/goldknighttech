...
import { ClickSoundsService } from '../click.sounds.service';
import { add, trashOutline, heart, mail, call,location,home,
  build,construct,hammer,desktop,diamond} from 'ionicons/icons';
...
export class DrawerPage implements AfterViewInit,OnDestroy{
  ...
  appPages: DrawerScreen[] = [
    ...
    { name: 'Service', icon: 'build', url: '/menu/service' },
    
    { name: 'Demo', icon: 'diamond', url: '/menu/demo' },
    { name: 'Contact', icon: 'call', url: '/menu/contact' },
...
//   private currentVolume = 0.5;
  ...
  constructor(
    ...
    //private audioService: HowlerAudioService
  ) {
  ...
  onDrawerNavigate(page: DrawerScreen) {
      this.clickSoundService.getSound()['click'].play()
    //this.sound.play()
    if (page.url) {
      this.activeTab = page.name;
    }
   // this.loadAndPlay("assets/sounds/click.wav")
...
// loadAndPlay(soundSrc:string): void {
//     this.audioService.load(soundSrc);  // Replace with your file
//     this.audioService.play();
//     setTimeout(() => {  // Fetch duration after load
...
//   }
  ...
  // toggleVolume(): void {
  //   this.currentVolume = this.currentVolume === 0.5 ? 1.0 : 0.5;
